...
/*   Updated: 2022/01/28 19:49:13 by wmozella         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_inizialization(t_vars *vars)
{
	...
	vars->img->img_win = mlx_xpm_file_to_image(vars->mlx, WIN,
			&vars->img->img_width, &vars->img->img_height);
	vars->img->img_enemy = mlx_xpm_file_to_image(vars->mlx, ENEMY,
			&vars->img->img_width, &vars->img->img_height);
	vars->img->img_lose = mlx_xpm_file_to_image(vars->mlx, LOSE,
			&vars->img->img_width, &vars->img->img_height);
	vars->img->img_open_exit = mlx_xpm_file_to_image(vars->mlx, OPEN_EXIT,
			&vars->img->img_width, &vars->img->img_height);
}
...
void	ft_img_render(t_vars *vars, int x, int y)
{
	...
	else if (vars->map[x][y] == 'A')
		mlx_put_image_to_window(vars->mlx, vars->win, vars->img->img_enemy,
			y * vars->size, x * vars->size);
	else if (vars->map[x][y] == 'C')
		mlx_put_image_to_window(vars->mlx, vars->win, vars->img->img_coin,
			y * vars->size, x * vars->size);
	else if (vars->map[x][y] == 'E' && vars->count_c != 0)
		mlx_put_image_to_window(vars->mlx, vars->win, vars->img->img_exit,
			y * vars->size, x * vars->size);
	else if (vars->map[x][y] == 'E' && vars->count_c == 0)
		mlx_put_image_to_window(vars->mlx, vars->win, vars->img->img_open_exit,
			y * vars->size, x * vars->size);
}
